# UrlShortener

Небольшой сервис для сокращения URL-адресов, разработанный для практики и изучения современных возможностей ASP.NET Core.

## 📋 О проекте

Это пет-проект, целью которого является поэтапное освоение и применение различных технологий и практик из экосистемы .NET.
Развитие проекта разделено на этапы, начиная с базового функционала и постепенно усложняя его.

**🚀 Текущий этап: Внедрение кэширования с использованием Redis (Этап 4)**

**Цель этапа:** Повышение производительности и масштабируемости приложения за счет кэширования часто запрашиваемых данных.

### Задачи текущего этапа:
- ✅ **Интеграция Redis:** Добавить и настроить библиотеки StackExchange.Redis и Microsoft.Extensions.Caching.StackExchangeRedis в проект
- ✅ **Настройка подключения:** Реализовать подключение к Redis серверу с поддержкой различных окружений
- ✅ **Реализация сервиса кэширования:** Создать методы расширения для встроенного кэширования
- ✅ **Кэширование операций чтения:** Добавить кэширование для операций получения сокращенных URL
- ✅ **Контейнеризация Redis:** Добавить Redis в Docker Compose конфигурацию

### Технологический стек текущего этапа:
- **Backend:** ASP.NET Core Web API
- **Язык:** C# (.NET 9.0)
- **База данных:** PostgreSQL
- **Кэширование:** Redis
- **ORM:** Entity Framework Core
- **Логирование:** Serilog
- **Архитектура:** Clean Architecture с разделением на слои
- **Контейнеризация:** Docker + Docker Compose

## 📁 Структура проекта

```
UrlShortener/                 # Solution
├── Shortener/                # Main project
│   ├── Controllers/          # Контроллеры API
│   ├── Data/                 # Контекст БД
│   ├── Entities/             # Сущности EF Core
│   ├── Models/               # Модели данных
│   │   └── DTOs/             # DTO-ответы
│   ├── Services/             # Бизнес-логика (сервисы)
│   ├── Extensions/           # Методы расширения
│   └── Dockerfile            # Docker образ приложения
├── docker-compose.yaml       # Docker Compose конфигурация
└── .env                      # Переменные окружения для Docker Compose
```

## 🛠 Установка и запуск

1. **Клонируйте репозиторий:**
    ```bash
    git clone https://github.com/your-username/UrlShortener.git
    cd UrlShortener
    ```

2. **Заполните .env файл по примеру из example.env**

3. **Запустите приложение с PostgreSQL и Redis:**
   ```bash
   docker-compose up --build
   ```

4. Приложение будет доступно по адресу: https://localhost:7000 (или http://localhost:5000)

## 🗺 Дорожная карта (Roadmap)

- [X] **Stage 1:** Базовый функционал (сокращение, редирект).
- [X] **Stage 2:** Добавление постоянного хранилища (например, PostgreSQL), Docker.
- [X] **Stage 3:** Добавление логгирования основных операций.
- [X] **Stage 4:** Внедрение кэширования (Redis) для увеличения производительности.
- [ ] **Stage 5:** Добавление пользовательского интерфейса (Frontend).
- [ ] **Stage 6:** Аутентификация, авторизация и личный кабинет с историей ссылок.
- [ ] **Stage 7:** Продвинутые функции: кастомные alias, аналитика переходов, TTL для ссылок.

---

## 🤝 Вклад в проект

Так как это учебный проект, любые предложения и советы по улучшению кода или архитектуры приветствуются! Не стесняйтесь создавать Issues или Pull Requests.